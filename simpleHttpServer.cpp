...
and return the response without PYTHON_PROMPT.
*/
    // get the response from python
  ...
  // strip off PYTHON_PROMPT (of length PYTHON_PROMPT_LEN) from the end of the response,
 totalBuffer = read(fromPythonFd, buffer, LINE_BUFFER_LEN - strlen(PYTHON_PROMPT_LEN));
...
static
pthread_mutex_t	safeRandLock__;
  ...
  bool		didPerhapsUpdate()
				{
				  ...
				  if  ( (statBuffer.st_mtimespec.tv_sec <
					 lastModificationTime_.tv_sec
					)
					||
					( (statBuffer.st_mtimespec.tv_sec  ==
					   lastModificationTime_.tv_sec
					  )
					  &&
					  (statBuffer.st_mtimespec.tv_nsec <=
...
protected :
  ...
  char*		getInstantiatedContent
				(size_t		instantiatedLength,
				 const Request&	request
				)
				const
				{
				  char*	cPtr;
				  char*	instantiatedContentsCPtr
						= (char*)
				...
				)
{
  ...
  while  (shouldContinue)
  {
	...
	throw BAD_REQUEST_HTTP_RET_CODE;
      ...
      toReturn	+= (char)
		   ((hexDigitValue(hiNibble) << 4) | hexDigitValue(loNibble));
      break;
				...
				)
{
  ...
  if  ( endsWith(filepathCPtr,".jpg") || endsWith(filepathCPtr,".jpeg") ||
      ...
      )
  {
    ...
    if  ( (memcmp(contentsCPtr,"\x47\x49\x46\x38\x37\x61",6) == 0)  ||
    ...
    if  ( (memcmp(contentsCPtr,"\xFF\xD8\xFF\xDB",4) == 0)		||
	  (memcmp(contentsCPtr,
    ...
    if  ( (memcmp(contentsCPtr,"\x00\x01\x00\x00",4) == 0)  ||
  ...
  if  ( endsWith(filepathCPtr,".html") || endsWith(filepathCPtr,".htm") )
  {
    toReturn	= DEFAULT_MIME_FORMAT;
  }
  else
  if  ( endsWith(filepathCPtr,".otf") )
  {
    toReturn	= "font/otf";
  }
  ...
  if  ( endsWith(filepathCPtr,".txt") )
  {
    toReturn	= "text/plain";
  }
  else
  if  ( endsWith(filepathCPtr,".ogv") )
  {
    toReturn	= "video/ogg";
  }
  else
				...
				curlHandle_(NULL)
{
  ...
  if  (pythonProgNameCPtr != NULL)
  {
    ...
    if(pipe(toPythonArray) < 0) {
        exit(EXIT_FAILURE);
    }
    if(pipe(fromPythonArray) < 0 ) {
        exit(EXIT_FAILURE);
    }
    pythonPid_ = fork();
    ...
    if  (pythonPid_  == 0)
    {
      ...
      execl("/usr/bin/python", "/usr/bin/python", "-u", pythonProgNameCPtr, NULL);
    }
...
//  PURPOSE:  To return the number of bytes in '*this' page given Request
//	'request'.
size_t		FixedFormPage::getNumBytes
				(const Request&	request
		 		)
{
  //  I.  Application validity check:
...
//  PURPOSE:  To translate the URI in 'uriCPtr' into the file path in
//	'filepathSpace' of length 'filepathSpaceLen'.  No return value.
void		PageStore::translateUriToFilepath
				(const char*	uriCPtr,
				 char* 		filepathSpace,
				 size_t		filepathSpaceLen,
				 std::map<std::string,std::string>&
				...
				)
{
  ...
  if  ( strncmp(filePath, SYM_EVAL_PAGE_NAME,strlen(SYM_EVAL_PAGE_NAME)) == 0)
  {
      ...
      if  (found != request.getQuery().end())
      {
        ...
        {
            const char*	cPtr;
            std::string xText = request.getQuery()["toEvaluate"];
            std::string yText = request.getQuery()["toEvaluate"];
            xText.c_str();
        }
  ...
  if  ( strcmp(filePath,"index.html") == 0)
  {
    ...
    std::map<std::string,std::string>::const_iterator
		varIter		= request.getQuery().
					find(USERNAME_HTML_VARNAME);
    ...
    std::string	id		= ( (varIter =
    				     request.getQuery().find(ID_HTML_VARNAME)
				    )
  ...
  if  ( strcmp(filePath,LOGOUT_PAGE_NAME) == 0)
  {
    const char*	cPtr;
  ...
  if  ( endsWith(filePath,SIMPLE_WEB_TEMPLATE_EXT) )
  {
...
//pthread_attr_setdetachstate(&pthreadAttr,PTHREAD_CREATE_DETACHED);
  ...
  InfoForClientServingThread	servingThreadInfo
					(content,
					 sessionStore,
					 pageStore,
					 newClientBuffer
  ...
  serverAddr.sin_addr.s_addr	= ((struct sockaddr_in*)hostPtr->ai_addr)->
					sin_addr.s_addr;
